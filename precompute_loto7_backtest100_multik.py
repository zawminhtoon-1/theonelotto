"""
precompute_loto7_backtest100_multik.py
-----------------------------------
Walk-forward backtest for Loto7, all 16 methods, over the LAST 100
real draws, evaluated at pick sizes K=7, 9, 13, 17 -- mirroring
backtest.html's actual architecture (not just its visual style):
each method's NATIVE K=15 candidate pool is computed once per draw
(walk-forward, no lookahead), embedded client-side, and topKNums()
(the same generic cross-method-consensus trim/pad port used
throughout this site, already proven to generalize to any K) derives
all four K views live in the browser -- no server-side recomputation
needed per K.
...
LSTM is the only method requiring the full sequential walk-forward
history (online learning is stateful) -- it walks the ENTIRE draw
history from the start for correct online training, but only the
last 100 draws' predictions are stored. All 15 other methods are
stateless and computed directly for just the last 100 target draws.

Output: loto7_backtest100_multik_data.json (consumed by
        gen_loto7_backtest100_multik.py to produce the actual page)
Run: python precompute_loto7_backtest100_multik.py
"""
...
if len(all_serials) < N_DRAWS_BACKTEST + 2:
    raise SystemExit(f"Not enough draws ({len(all_serials)}) for a {N_DRAWS_BACKTEST}-draw backtest.")

start_idx = len(all_serials) - N_DRAWS_BACKTEST
print(f"Backtest window: draws #{all_serials[start_idx]}-{all_serials[-1]} ({N_DRAWS_BACKTEST} draws)")
...
# ── LSTM: full sequential walk-forward online training (stateful) ───────────
...
print(f"LSTM online walk-forward through draws #{all_serials[WARMUP]}-#{all_serials[start_idx-1]} "
      f"(state build-up before the backtest window; predictions not stored)...")
t_lstm1 = time.time()
lstm_pool_by_idx = {}
for idx in range(WARMUP, len(all_serials)):
    xs = lstm_vecs[idx-SEQ:idx] if idx >= SEQ else []
    if idx >= SEQ:
        pool = lstm.predict_pool(xs)
        if idx >= start_idx:
            lstm_pool_by_idx[idx] = pool
        lstm.train_step(xs, lstm_vecs[idx])
print(f"LSTM full walk done in {time.time()-t_lstm1:.1f}s")

# ── 15 stateless methods: computed directly for the last 100 target draws ───
def compute_hits_and_bonus(pool, actual7, b1, b2, k):
    combo_full = pool[:k] if len(pool) >= k else pool
    hits = len(set(combo_full) & set(actual7))
    bonus_hit = (b1 in combo_full) or (b2 in combo_full)
    return hits, bonus_hit

print(f"\nComputing 15 stateless methods natively (K={POOL_K}) for {N_DRAWS_BACKTEST} draws...")
t0 = time.time()
DATA = []
for i, idx in enumerate(range(start_idx, len(all_serials)), 1):
    ...
    pools = [
        ...
        lstm_pool_by_idx.get(idx, list(range(1, POOL_K+1))),
    ]
    for p in pools:
    ...
    if i % 20 == 0 or i == N_DRAWS_BACKTEST:
        elapsed = time.time() - t0
        rate = i / elapsed
        eta = (N_DRAWS_BACKTEST - i) / rate if rate > 0 else 0
        print(f"  [{i}/{N_DRAWS_BACKTEST}] draw #{target_serial} elapsed={elapsed:.0f}s rate={rate:.2f} draws/s eta={eta:.0f}s", flush=True)

print(f"Done in {time.time()-t0:.1f}s")

# ── Server-side aggregate (Python reference for self-check; the page itself
# computes all 4 K views live client-side via topKNums(), same as backtest.html) ──
...
print("\n=== Self-check: server-side reference computation for K=7 ===")
hc7, h7b7 = compute_for_k(7)
ranked = sorted(range(16), key=lambda mi: (-h7b7[mi], -hc7[mi][7], -hc7[mi][6], -hc7[mi][5], -hc7[mi][4]))
print("Rank @ K=7 (hit7b -> hit7 -> hit6 -> hit5 -> hit4):")
for mi in ranked:
    print(f"  {METHOD_NAMES[mi]:24s} hit7b={h7b7[mi]} hit7={hc7[mi][7]} hit6={hc7[mi][6]} hit5={hc7[mi][5]} hit4={hc7[mi][4]}")

for K in K_OPTIONS:
    hc, h7b = compute_for_k(K)
    ranked_k = sorted(range(16), key=lambda mi: (-h7b[mi], -hc[mi][7], -hc[mi][6], -hc[mi][5], -hc[mi][4]))
    best = ranked_k[0]
    print(f"\nK={K}: best method = {METHOD_NAMES[best]} "
...
DATA_JSON_OUT = BASE + r"\loto7_backtest100_multik_data.json"
with open(DATA_JSON_OUT, 'w', encoding='utf-8') as f:
    json.dump({'methods': METHOD_NAMES, 'colors': COLORS, 'kOptions': K_OPTIONS, 'data': DATA}, f, separators=(',', ':'))
