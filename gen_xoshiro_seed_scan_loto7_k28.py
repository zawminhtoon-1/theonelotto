"""
gen_xoshiro_seed_scan_loto7_k28.py
--------------------------------------
Static report page for Loto7's xoshiro256** K=28 seed scan: seeds
-1,000,000 to 1,000,000 (2,000,001 seeds), K=28 picks, backtested
against the FIRST 500 Loto7 draws (#1-500) -- direct reuse of the
K=25 scan's infrastructure and page pattern
(gen_xoshiro_seed_scan_loto7_k25.py), just K_PICKS changed from 25 to
28 and top-5 (not top-3) best-seed summary, per explicit instruction.

Run directly at the full +/-1,000,000 range (no narrower starting
range this time, since the K=25 infra was already proven at this
scale) via xoshiro_seed_scan_loto7_k28_stage.py, staged in 4
~500,000-seed batches (same staged negative-seed pattern as the Loto6
K=35 scan and the Loto7 K=25 scan), each stage self-checked
(including negative seeds) before running.

Reads live from loto7_local.db's seed_hit_xoshiro_k28 table for
...
Output: public/xoshiro_seed_scan_loto7_k28.html
Run: python gen_xoshiro_seed_scan_loto7_k28.py
"""
...
HTML_OUT = BASE + r"\public\xoshiro_seed_scan_loto7_k28.html"
TABLE = "seed_hit_xoshiro_k28"
...
print(f"Top 5: {[(r[0], r[1], r[2], r[3], r[4], r[5]) for r in top5]}")
...
# ── Table rows ────────────────────────────────────────────────────────────
def render_rows(rows, highlight_seed=None, top5_seeds=()):
    html = ""
    for rank, (seed, hit7b, hit7, hit6, hit5, hit4) in enumerate(rows, 1):
        badge = ' <span class="badge">BEST</span>' if seed == highlight_seed else (' <span class="badge3">TOP 5</span>' if seed in top5_seeds and seed != highlight_seed else '')
...
top5_seed_set = set(r[0] for r in top5)
rows_html = render_rows(top_ranked, highlight_seed=best[0], top5_seeds=top5_seed_set)

top5_summary_html = ""
for i, row in enumerate(top5, 1):
    top5_summary_html += f"""<div class="stat-card{' final' if i == 1 else ''}">
...
<title>Xoshiro Seed Scan K=28 (-1,000,000 to 1,000,000) — Loto 7</title>
...
<div class="wrap">
  <h1>🎯 Xoshiro Seed Scan — Loto 7 K=28 (&minus;1,000,000 to 1,000,000)</h1>
  ...
  <div class="note">
    Reuses the <a href="/xoshiro_seed_scan_loto7_k25.html" style="color:#a78bfa">Loto7 K=25 scan</a>'s infrastructure
    directly at the full {SEED_LO:,} to {SEED_HI:,} range ({num_seeds:,} seeds, K={K_PICKS} instead of K=25) &mdash;
    ...
    {N_DRAWS} Loto7 draws (#{DRAW_START}&ndash;{DRAW_END}) &mdash; not the most recent.
    <br><br>
  ...
  <div class="section">
    ...
    <div class="stats-row">{top5_summary_html}</div>
  </div>
