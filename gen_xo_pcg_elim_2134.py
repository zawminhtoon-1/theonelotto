  ...
  <div class="section">
    ...
    <div id="comboUI" style="display:none">
      ...
      <div class="lookup" style="margin-top:4px">
        <span class="pd-lbl">Random sample</span>
        ...
        <span class="page-info">Samples from the currently filtered set (or all {final_remaining:,} if no filter is active).</span>
      </div>
...
// ── Random sample generator: draws N distinct combos from the currently
// filtered set (or all REMAINING if no filter is active) via a partial
// Fisher-Yates shuffle of index positions (doesn't mutate filtered/REMAINING). ─
function generateSamples(n) {{
  ...
  const picks = indices.slice(indices.length - count).map(i => pool[i]);
  ...
  container.innerHTML =
    '<div class="gen-hdr">Generated ' + picks.length + ' random combo' + (picks.length !== 1 ? 's' : '') + ' from ' + sourceLabel + ':</div>' +
