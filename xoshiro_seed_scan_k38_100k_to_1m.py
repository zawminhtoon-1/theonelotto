"""
xoshiro_seed_scan_k38_100k_to_1m.py
---------------------------------------
Stage 2 of the K=38 scan: seeds 100,001-1,000,000 (900,000 seeds), same
K=38 xoshiro256** algorithm/formula, same draw window #1000-2127 (1128
draws), same hit6b/hit6/hit5 metrics and ranking as Stage 1. Adds to
(not replaces) Stage 1's seeds 0-100,000 -- combined, this covers the
full 0-1,000,000 range.

Stage 1 (100,001 seeds) ran at ~51.75 seeds/s actual (slower than K=33's
equivalent leg) -- this stage is estimated at ~4.8 hours at that rate.
Kept at 7 workers, same reasoning as the K=33 scan (other processes
running concurrently on this machine).

Run: python xoshiro_seed_scan_k38_100k_to_1m.py
"""
...
OUT_JSON = BASE + r"\xoshiro_seed_scan_k38_100k_to_1m.json"

K_PICKS = 38
DRAW_START, DRAW_END = 1000, 2127
N_DRAWS = DRAW_END - DRAW_START + 1  # 1128
LOTO6_MAX = 43
SEED_LO, SEED_HI = 100_001, 1_000_000
N_WORKERS = 7
CHUNK_SIZE = 200
...
def main():
    ...
    for test_seed, test_draw in [(100001, 1000), (168, 2127), (999999, 1500), (555555, 2000), (1000000, 2127)]:
