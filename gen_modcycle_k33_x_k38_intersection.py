"""
gen_modcycle_k33_x_k38_intersection.py
--------------------------------------
Static report page for the Modular Cycle (K=33) x xoshiro K=38 seed
#692,809 intersection backtest over the last 100 draws. Direct mirror of
gen_modcycle_k28_x_k38_intersection.py (same structure, style, 3+/6 hit
threshold, hypergeometric proportionality baseline, distinct gold 6/6
badge, two-stats-row layout, look-ahead-bias caveats) -- just K=33 for
the Modular Cycle side instead of K=28. topKNums() supports any K, not
just the backtest.html toggle button values (confirmed live before this
page was built).

One fix applied here vs the K=28 template: its <h1> hardcoded the literal
text "K=28" instead of using the K_MC variable -- harmless there since it
happened to match, but would have silently shown the wrong K on this
page. Parameterized properly here.

Reads modcycle_k33_x_k38_intersection_data.json, produced by
compute_modcycle_k33_x_k38_intersection.js (which extracts Modular
...
Output: public/xoshiro_k38_x_modularcycle_k33_intersection.html
Run: node compute_modcycle_k33_x_k38_intersection.js
     python gen_modcycle_k33_x_k38_intersection.py
"""
import json
from math import comb

BASE = r"C:\Users\Zaw Min Htoon\source\repos\theonelotto"
DATA_JSON = BASE + r"\modcycle_k33_x_k38_intersection_data.json"
NEXT_DRAW_JSON = BASE + r"\modcycle_next_draw_x_k38_data.json"
HTML_OUT = BASE + r"\public\xoshiro_k38_x_modularcycle_k33_intersection.html"
...
next_draw_block = next_draw_payload['k33']
...
<title>Modular Cycle (K=33) x K=38 Intersection Backtest — Loto 6</title>
<style>
...
<div class="wrap">
  <h1>&#9986;&#65039; Modular Cycle (K={K_MC}) &times; K=38 Seed #{XO_SEED:,} Intersection</h1>
  ...
  <div class="note info">
    ...
    on <a href="/backtest.html" style="color:#7dd3fc">backtest.html</a> (its own K={K_MC} pick, padded/trimmed via the
    same cross-method-consensus <code>topKNums()</code> logic that page uses -- it works for any K, not just the
    page's own toggle button values); the other side is xoshiro256** seed #{XO_SEED:,} (the top-ranked K=38 seed by
    hit6b), computed fresh per draw from that draw's own draw_serial. Every row recomputes both picks and intersects
    them for that specific draw.</p>
