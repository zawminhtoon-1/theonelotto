/*
 * compute_modcycle_k33_x_k38_intersection.js
 * --------------------------------------------
 * Precomputes the Modular Cycle (K=33) x xoshiro K=38 seed #692,809
 * intersection backtest over the last 100 draws, for
 * gen_modcycle_k33_x_k38_intersection.py to render into a static page.
 * Direct mirror of compute_modcycle_k28_x_k38_intersection.js, just K=33
 * instead of K=28 for the Modular Cycle side (topKNums() supports any K,
 * not just the backtest.html toggle button values -- confirmed live
 ...
 * bit-exact with what the live page shows. Note: the METHODS array label
 * is literally the string "Modular Cycle (k=28)" regardless of which K is
 * passed to topKNums() here -- that's just backtest.html's display name
 * for the method slot, not tied to the K value being tested.
 *
 ...
 * Output: modcycle_k33_x_k38_intersection_data.json
 */
const fs = require('fs');

const BASE = 'C:\\Users\\Zaw Min Htoon\\source\\repos\\theonelotto';
const BACKTEST_HTML = BASE + '\\public\\backtest.html';
const OUT_JSON = BASE + '\\modcycle_k33_x_k38_intersection_data.json';

const K_MC = 33;
const K_XO = 38;
const XO_SEED = 692809;
const MC_METHOD_NAME = 'Modular Cycle (k=28)'; // display label in METHODS, not tied to K_MC
