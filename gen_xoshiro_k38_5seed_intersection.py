...
(originally this page required a strict 6-of-6 full-subset match; changed
per request after a user was confused why #2129 -- which had 3 of 6
matches, 16/25/41 -- showed as no-hit under the old all-or-nothing
definition). The per-draw match count (0-6) is shown as its own column.

Proportionality baseline: since pool size varies per draw (range ~20-27),
the "expected under pure chance" rate is computed per-draw via the
hypergeometric distribution -- P(X>=3) where X = count of a random
6-number draw's numbers landing in a population-43, success-states-m
pool, m = that draw's actual intersection pool size -- then averaged
across the 100 draws. This replaces the old "avg pool combo count / full
universe" baseline, which only made sense for the strict 6-of-6
definition.
...
SEEDS = [692809, 394299, 129218, 920766, 806181]
...
<div class="wrap">
  ...
  <p class="subtitle">Top 5 K=38 seeds by hit6b &middot; intersection pool per draw &middot; last {N_BACKTEST_DRAWS} draws (#{draw_lo}&ndash;{draw_hi}) &middot; hit = {HIT_THRESHOLD}+ of 6 numbers in pool</p>

  <div class="note info">
    <p><strong>Hit definition: {HIT_THRESHOLD}+ of 6 (partial match).</strong> A draw counts as a hit if at least
    {HIT_THRESHOLD} of its 6 winning numbers land inside that draw's intersection pool -- not all 6. (This page
    previously required a strict 6-of-6 full-subset match; changed after a user asked why draw #2129, which had
    3 of 6 matches, showed as no-hit under that all-or-nothing definition.) Every row below shows the exact match
    count out of 6, and matched numbers are highlighted green in both the actual-numbers and intersection-numbers
    columns.</p>
  </div>
  ...
  <div class="section">
    ...
    <p class="desc">Newest first. Each row recomputes all 5 seeds' K=38 picks using that draw's own draw_serial,
  ...
  <p class="footer">
    ...
    Intersection pool = numbers appearing in ALL 5 seeds' 38-number picks for that specific draw. Hit = at least
