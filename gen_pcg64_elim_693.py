...
function applyFilter() {{
  ...
  // or any badge -- purely restricts what's displayed/sampled here. Pure
  // per-box EXCLUSION (not an allowlist): unchecking a box hides ONLY
  // combos with that exact pair-count. 0-pair combos have no checkbox of
  // their own and are NEVER excluded, regardless of what's checked. All
  // three boxes checked (the default) means "no filtering" -- everything,
  // including 0-pair combos, stays visible.
  ...
  const excludedPairCounts = new Set([...(p1 ? [] : [1]), ...(p2 ? [] : [2]), ...(p3 ? [] : [3])]);
  ...
  filtered = (includeNums.length === 0 && excludeNums.length === 0 && hcVal === '' && !pairsFilterActive) ? REMAINING : REMAINING.filter(c => {{
    ...
    if (pairsFilterActive && excludedPairCounts.has(consecutivePairCount(c))) return false;
    return true;
  }});
  const parts = [];
  ...
  if (pairsFilterActive) parts.push('exclude ' + [...excludedPairCounts].sort().map(k => k + '-pair').join(', ') + ' combos');
