"""
precompute_xoshiro_elim_2134.py
-----------------------------------
Precomputes everything the "Xoshiro K=38 x Modular Cycle Native K=38 +
16-Method Elimination" page needs for draw #2134 -- the true next-
upcoming Loto6 draw (#2133 landed 2026-08-31: main 1,11,14,20,29,38,
bonus 27, ingested and cross-verified against Mizuho's official CSV
before this build). Standard walk-forward build, trained on all real
draws through #2133, no leakage.

METHODOLOGY CHANGE from #2133's page: Base here is xoshiro K=38 seed
#692,809's pick intersected with Modular Cycle's NATIVE mod-43-cycle
K=38 pick (no cross-method-consensus padding) -- the same construction
tested on xoshiro_k38_x_modularcycle_k38_stats.html, NOT the K=33
padded intersection used on xoshiro_elim_2133.html. Rationale: #2133's
actual result (1,11,14,20,29,38 + bonus 27) was a full hit6b for this
K38-native-intersect construction (all 7 numbers, including bonus,
were in the pool) while it was a complete MISS for the K=33-padded
construction (number 1 wasn't even in that 28-number pool) -- a
striking single data point, though the aggregate walk-forward backtest
on the stats page still shows this construction is NOT statistically
distinguishable from chance (avg pool ~33.6, none of hit6b/hit6/hit5/
hit4 reach p<0.05 over 2,089 backtested draws). Included here as an
experiment, with that caveat carried into the page copy.
...
(consecutive-run filter), Pass 6 (three-consecutive-pairs filter): all
IDENTICAL methodology to xoshiro_elim_2133.html, just walk-forward
through #2133 instead of #2132. Passes 7-10 (the 3/4/5-overlap-vs-
recent-draws filters used on #2133's page) are DELIBERATELY OMITTED --
this session's own statistical follow-up found none of those reached
significance (binomial p=0.071 for Pass 7's core claim; the pooled
100,000-comparison analysis behind Pass 10 landed within hundredths of
a percent of the exact chance rate).

Pass 7 (new, final): a DIFFERENTLY-JUSTIFIED overlap filter -- removes
any Pass-6-remaining combo that shares 5 or 6 numbers with the
immediately previous actual draw (#2133: 1,11,14,20,29,38). Unlike the
omitted 3/4/5-based passes, this one has a clean, well-supported basis:
across all 2,131 consecutive Loto6 draw pairs in the site's full
recorded history (#1-2132), an overlap of 5 or 6 numbers has NEVER
happened -- 0 occurrences, not "somewhat rarer than chance" or "not
quite significant." This is the strongest empirical exclusion basis
used on any elimination page on this site.

Self-checks the xoshiro implementation against a known-good value
before trusting Base's xoshiro component.

Outputs:
  xoshiro_elim_2134_meta.json          -- small: pools, method picks, counts
  public/xoshiro_elim_2134_combos.json -- large: remaining combo list
                                          (fetched client-side, not inlined)

Run: python precompute_xoshiro_elim_2134.py
"""
...
META_OUT = BASE + r"\xoshiro_elim_2134_meta.json"
COMBOS_OUT = BASE + r"\public\xoshiro_elim_2134_combos.json"

LOTO6_MAX = 43
TARGET_SERIAL = 2134
K_XO = 38
K_MC = 38        # NATIVE Modular Cycle K for Base -- no cross-method padding
...
# ── Base: xoshiro K=38 seed #692,809 ∩ Modular Cycle NATIVE K=38 ────────────
# (no cross-method padding -- direct native ranking, unlike #2133's page)
mc_pool_ordered = modular_cycle_ranked(train_serials, train_main6, TARGET_SERIAL, k=K_MC)
mc_pool = sorted(mc_pool_ordered)
print(f"\nModular Cycle NATIVE K={K_MC} pick for draw #{TARGET_SERIAL}: {mc_pool}")
print(f"  (generation order): {mc_pool_ordered}")

base_pool = sorted(set(mc_pool) & set(xo_pool))
K_BASE = len(base_pool)
print(f"\nBase (xoshiro K={K_XO} ∩ Modular Cycle NATIVE K={K_MC}): {K_BASE} numbers: {base_pool}")
...
# ── Pass 7 (final): "5 or 6 overlap with the immediately previous draw"
# filter. Removes any Pass-6-remaining combo that shares 5 or 6 numbers
# with #2133 (1,11,14,20,29,38). Well-supported basis (unlike the 3/4/5-
# based passes omitted from this page): across ALL 2,131 consecutive Loto6
# draw pairs in full recorded history (#1-2132), overlap of 5 or 6 has
# NEVER happened -- 0 occurrences, verified independently this session.
# (Overlap of exactly 6 would mean an exact repeat of #2133, already
# excluded by Pass 3's historical filter -- included here anyway for an
# explicit, self-contained pass.) ───────────────────────────────────────────
print(f"\n=== Pass 7 (final) ===")
...
if removed_by_pass7:
    print(f"  Removed combos: {removed_by_pass7}")
