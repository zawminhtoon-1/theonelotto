"""
gen_xoshiro_elim_2131.py
----------------------------
Generates the "Xoshiro K=38 x Modular Cycle K=33 + 16-Method Elimination"
page for draw #2131 -- NOT the true next-upcoming draw (that's #2130,
since #2129 is still the latest real/confirmed draw as of this build;
see xoshiro_elim_2130.html). Built anyway per explicit user request, as
a genuinely separate one-step-further target trained on the same #1-2129
window as #2130 (see precompute_xoshiro_elim_2131.py's docstring for why
the output still differs meaningfully from #2130's). Reads
xoshiro_elim_2131_meta.json (small: pool picks, method picks, counts)
produced by precompute_xoshiro_elim_2131.py. The large remaining-combo
list lives separately at public/xoshiro_elim_2131_combos.json (already
written by the precompute script) and is fetched client-side, not inlined.
...
and xoshiro K=38 seed #692,809's pick, both for #2131 -- same 28-number
pool already shown on xoshiro_k38_x_modularcycle_k33_intersection.html's
"next upcoming draw" section. Only the xoshiro component is recomputed
live in the browser (bit-exact BigInt port, verified against the
server-embedded reference); Modular Cycle's pick is embedded static data
computed server-side, same as every other ML-based method on this site.
Base (the intersection) is then computed client-side from the live
xoshiro result and the embedded Modular Cycle pool, and also verified
against a server-embedded reference.
...
precompute_xoshiro_elim_2131.py's docstring for the full mapping. Reads
directly from the rebuilt meta.json's pass2K/pass2Seeds/... (formerly
pass3...) / pass4K/pass4Pick/... (formerly pass5...) keys.

Output: public/xoshiro_elim_2131.html
Run: python gen_xoshiro_elim_2131.py
"""
import json

BASE = r"C:\Users\Zaw Min Htoon\source\repos\theonelotto"
META_PATH = BASE + r"\xoshiro_elim_2131_meta.json"
HTML_OUT = BASE + r"\public\xoshiro_elim_2131.html"

with open(META_PATH, encoding='utf-8') as f:
    meta = json.load(f)

TARGET_SERIAL = meta['targetSerial']
TRAINED_THROUGH = meta['trainedThroughSerial']  # 2129 -- NOT TARGET_SERIAL-1 (=2130); see precompute docstring
xo = meta['xo']
mc = meta['mc']
base = meta['base']
...
<div class="wrap">
  ...
  <div class="note">
    <p style="color:#fbbf24"><strong>Note:</strong> #{TARGET_SERIAL} is <em>not</em> the true next-upcoming Loto 6 draw &mdash; that's
    <a href="/xoshiro_elim_2130.html" style="color:#a78bfa">#2130</a> (draw #{TRAINED_THROUGH} is still the latest real/confirmed
    result). This page targets one draw further out, deliberately, using the identical #1&ndash;{TRAINED_THROUGH} training window as
    #2130 (there's no newer data to train on yet). That does <em>not</em> make its output a copy of #2130's, though: the target
    serial itself feeds directly into several formulas below as a live input, not just a training-window cutoff &mdash; xoshiro256**'s
    PRNG seed is <code>seed&times;10,000,000 + draw_serial</code>, Modular Cycle groups history by <code>draw_serial % 43</code>,
    Poly Regression extrapolates its fitted curve at <code>draw_serial</code>, and Random Forest takes <code>[draw_serial]</code> as
    its input feature &mdash; so Base and several of the 16 methods below are genuinely #{TARGET_SERIAL}-specific, not #2130's numbers
    relabeled.</p>
    <p><strong style="color:#e2e8f0">Base</strong> (below) is <strong>Modular Cycle's K={mc['k']} pick</strong> intersected with
    <strong>xoshiro256** K={xo['k']} seed #{xo['seed']:,}'s pick</strong> &mdash; both computed specifically for draw #{TARGET_SERIAL}
    (not read off the <a href="/xoshiro_k38_x_modularcycle_k33_intersection.html" style="color:#a78bfa">Modular Cycle × K=38
    intersection page</a>, which only ever shows the true next-upcoming draw, #2130). It defines the working universe: all
    C({base['k']},6) = {universe_count:,} six-number combinations drawable from this pool.</p>
    ...
    <p><strong style="color:#e2e8f0">Pass 4</strong> (final) is the Worst Combo (Anti-Pick) K={pass4_k} pick for draw #{TARGET_SERIAL}
    ...
    contained within this {pass4_k}-number pick gets removed. These 5 methods can't all run client-side, so the pick is embedded as
    static data (same convention as Modular Cycle's pick) &mdash; but unlike #2130 (where this pick was a snapshot read directly off
    the live /predictions page), /predictions always shows the true next-upcoming draw (#2130), not #{TARGET_SERIAL}, so this pick
    is instead computed directly from the same 5 methods' native picks above (indices into Pass 1's 16-method table) using the same
    union-count-desc-then-ascending-number combining rule as the Predictions page's own consensus logic.</p>
    <p>The xoshiro side of Base and all {len(pass2_seeds)} Pass-2 picks are recomputed
    <strong>live in your browser</strong> below (bit-exact BigInt xoshiro256** port) and checked against server-embedded references
    &mdash; check the verification badges. Modular Cycle's pick, Pass 1's 16 statistical/ML methods (ARIMA, Random Forest, HMM, LSTM,
    etc.), and Pass 4's Worst Combo pick can't run in a browser, so those are precomputed server-side, same as every other draw on
    this site, and embedded as static data. Pass 3's historical combo set is embedded and checked client-side too.</p>
  </div>
  ...
  <div class="section">
    <h2>Pass 4 (final) — Worst Combo (Anti-Pick), K={pass4_k} pick for draw #{TARGET_SERIAL} <span class="verify-badge na">server-computed</span></h2>
    <p class="desc">Same 5-method combination as <a href="/predictions" style="color:#a78bfa">the Predictions page's</a> Worst Combo
    panel &mdash; MA-43 + Exp-weighted + Random Forest + kNN + Apriori Association Rules consensus &mdash; but computed directly here
    for #{TARGET_SERIAL} rather than read off that page (which only ever targets the true next-upcoming draw, #2130). Overlap with
    the {base['k']}-pool: {pass4_overlap} numbers.</p>
