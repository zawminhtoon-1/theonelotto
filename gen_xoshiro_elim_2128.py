"""
gen_xoshiro_elim_2128.py
----------------------------
Generates the "Xoshiro K=33/K=26 + 16-Method Elimination" page for draw
#2128 (next upcoming Loto6 draw). Reads xoshiro_elim_2128_meta.json
(small: block picks, method picks, counts) produced by
precompute_xoshiro_elim_2128.py. The large remaining-combo list lives
separately at public/xoshiro_elim_2128_combos.json (already written by
the precompute script) and is fetched client-side, not inlined --
1,007,779 combos would bloat this HTML page itself to an unusable size.

Block 1 (xoshiro K=33) and Block 2 (xoshiro K=26) are recomputed LIVE
in the browser via the same bit-exact BigInt xoshiro256** port used on
every other xoshiro page -- verifiable, not just displayed. Block 3
(the 16 statistical/ML methods) are precomputed server-side, same as
every other draw on this site (ARIMA/RandomForest/HMM/LSTM etc. aren't
things a browser should redundantly recompute) -- embedded as static
data.

Output: public/xoshiro_elim_2128.html
Run: python gen_xoshiro_elim_2128.py
"""
import json

BASE = r"C:\Users\Zaw Min Htoon\source\repos\theonelotto"
META_PATH = BASE + r"\xoshiro_elim_2128_meta.json"
HTML_OUT = BASE + r"\public\xoshiro_elim_2128.html"
...
method_picks_21 = meta['methodPicks21']
universe_count = meta['universeCount']
removed_by_block2 = meta['removedByBlock2']
after_block2 = meta['afterBlock2']
...
methods_json = json.dumps(method_picks_21, separators=(',', ':'))
method_names_json = json.dumps(method_names, separators=(',', ':'))
...
<title>Xoshiro K=33/K=26 + 16-Method Elimination — Draw #{TARGET_SERIAL}</title>
<style>
...
<div class="wrap">
  <h1>✂️ Xoshiro K=33/K=26 + 16-Method Elimination — Draw #{TARGET_SERIAL}</h1>
  <p class="subtitle">Combinatorial set-difference: xoshiro256** K=33 pick, minus combos covered by xoshiro K=26's pick and by any of the 16 prediction methods' K=21 picks</p>

  <div class="note">
    <p><strong style="color:#e2e8f0">Block 1</strong> (below) is seed <strong>#{block1['seed']:,}</strong>'s xoshiro256** K={block1['k']} pick &mdash;
    the best seed found in the 0&ndash;1,000,000 K=33 scan (highest hit6b/hit6/hit5 ranking). This defines the working universe:
    all C(33,6) = {universe_count:,} six-number combinations drawable from its 33-number pool.</p>
    <p><strong style="color:#e2e8f0">Block 2</strong> is seed <strong>#{block2['seed']:,}</strong>'s xoshiro256** K={block2['k']} pick &mdash;
    the <em>worst</em> seed found in the 0&ndash;100,000 K=26 scan. Any of Block 1's combos fully contained within this 26-number
    set gets removed.</p>
    <p><strong style="color:#e2e8f0">Block 3</strong> is each of the 16 prediction methods' K=21 pick for draw #{TARGET_SERIAL}, computed
    walk-forward (trained on all {TARGET_SERIAL-1:,} real draws through #{TARGET_SERIAL-1}) then normalized to exactly 21 numbers via the same
    ...
    Any of Block 1's remaining combos fully contained within ANY single one of these 16 sets also gets removed.</p>
    <p>Blocks 1 &amp; 2 (xoshiro) are recomputed <strong>live in your browser</strong> below &mdash; check the verification badges.
    Block 3's 16 statistical/ML methods (ARIMA, Random Forest, HMM, LSTM, etc.) are precomputed server-side, same as every other
    draw on this site, and embedded as static data.</p>
  </div>

  <div class="section">
    <h2>Block 1 — xoshiro256** K={block1['k']}, seed #{block1['seed']:,} <span id="badge1" class="verify-badge pending">verifying…</span></h2>
    <p class="desc">Best K=33 seed (0–1,000,000 scan, ranked by hit6b→hit6→hit5). This 33-number pool is the elimination universe.</p>
    <div class="balls" id="block1Balls"></div>
  </div>

  <div class="section">
    <h2>Block 2 — xoshiro256** K={block2['k']}, seed #{block2['seed']:,} <span id="badge2" class="verify-badge pending">verifying…</span></h2>
    <p class="desc">Worst K=26 seed (0–100,000 scan, ranked lowest hit6b→hit6→hit5). Combos it fully covers get removed from Block 1's universe.</p>
    <div class="balls" id="block2Balls"></div>
  </div>

  <div class="section">
    <h2>Block 3 — 16 prediction methods, K=21 pick for draw #{TARGET_SERIAL}</h2>
    <p class="desc">Precomputed server-side (walk-forward, trained on all real draws through #2127), normalized to K=21 via cross-method consensus.</p>
    <details>
  ...
  <div class="section">
    ...
    <div class="stats-row">
      <div class="stat-card">
        <div class="lbl">Universe (Block 1)</div>
        <div class="val">{universe_count:,}</div>
        <div class="sub">C(33,6)</div>
      </div>
      <div class="stat-card">
        <div class="lbl">Removed by Block 2</div>
        <div class="val">{removed_by_block2:,}</div>
        <div class="sub">contained in the 26-set</div>
      </div>
      <div class="stat-card">
        <div class="lbl">After step 1</div>
        <div class="val">{after_block2:,}</div>
        <div class="sub">remaining after Block 2</div>
      </div>
      <div class="stat-card">
        <div class="lbl">Removed by 16 methods</div>
        <div class="val">{removed_by_methods:,}</div>
        <div class="sub">contained in ANY method's K=21</div>
      </div>
    ...
    <div class="elim-flow">
      ...
      <span class="n">{after_block2:,}</span>
      <span class="arrow">&rarr;</span>
...
// ── Block 1 & 2: compute live + verify against server-embedded reference ────
const KNOWN_BLOCK1 = {json.dumps(block1['pool'])};
const KNOWN_BLOCK2 = {json.dumps(block2['pool'])};

const liveBlock1 = xoshiroPredict({block1['seed']}, {TARGET_SERIAL}, {block1['k']});
const liveBlock2 = xoshiroPredict({block2['seed']}, {TARGET_SERIAL}, {block2['k']});
...
if (!arraysEqual(liveBlock1, KNOWN_BLOCK1)) console.error('Block1 mismatch', liveBlock1, KNOWN_BLOCK1);
if (!arraysEqual(liveBlock2, KNOWN_BLOCK2)) console.error('Block2 mismatch', liveBlock2, KNOWN_BLOCK2);
...
fetch('/xoshiro_elim_2128_combos.json')
  .then(r => r.json())
  .then(data => {{
    REMAINING = data;
    filtered = REMAINING;
