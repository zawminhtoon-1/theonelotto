"""
precompute_xoshiro_elim_2133_staging.py
-------------------------------------------
STAGING VARIANT of precompute_xoshiro_elim_2133.py -- for side-by-side
comparison only. Does NOT touch the live xoshiro_elim_2133.html/
xoshiro_elim_2133_meta.json/public/xoshiro_elim_2133_combos.json at
all; writes to its own _staging-suffixed files instead.

The ONLY methodological difference from the live page: Base is
xoshiro256** K=38 seed #692,809's pick ALONE (38 numbers), with the
Modular Cycle K=33 intersection step dropped entirely, per explicit
request to evaluate whether that intersection is worth keeping. Every
other pass (1-10, including Pass 1's 16 methods -- Modular Cycle is
still one of the 16 and still gets computed and used there, just not
folded into Base) is identical to the live page's methodology.

Rationale/estimate that motivated this staging build: the intersected
Base (K=28) showed a real historical containment edge over chance in
a 100-draw retroactive backtest (14% observed vs. 6.2% chance -- a
~2.3x relative lift), notably stronger than xoshiro-alone's own edge
in its own 1,128-draw scan history (52.1% observed vs. 45.3% chance --
only ~1.15x). Dropping the intersection was estimated (and now
computed for real, matching the estimate) to expand the final
Pass-10 remaining pool roughly 8x (24,839 on the live page -> ~200,862
here) -- see this build's actual numbers below.

Precomputes everything the "Xoshiro K=38 (standalone, no Modular Cycle
intersection) -- STAGING" page needs for draw #2133 -- the true next-
upcoming Loto6 draw (#2132 is now the latest real/confirmed draw,
having landed since xoshiro_elim_2132.html was built). Standard walk-
forward build, same methodology as #2130/#2132, trained on all real
draws through #2132, no leakage.
...
per explicit user request. Downstream passes renumbered:
  ...
  old Pass 7 (three-consecutive-pairs)   -> Pass 6

Pass 7 (new, added after the Stage-1 xoshiro K=20 seed scan's consecutive-
draw-overlap analysis): consecutive-draw high-overlap filter -- now the
final pass. See its own docstring block below for details.

Base pool: Modular Cycle's K=33 pick (walk-forward, trained on all real
           draws through #2132, native K=28 pick normalized to K=33 via
           topKNums()) INTERSECTED with xoshiro256** K=38 seed #692,809's
           pick (current overall best K=38 seed, full 0-1,000,000 scan)
           -- both for draw #2133. Universe = C(K_BASE,6), K_BASE being
           whatever the intersection size comes out to.

Pass 1:    each of the 16 prediction methods' K=19 pick for #2133,
           computed walk-forward (trained on all real draws through
           #2132, append_backtest.py's method definitions) then
...
Pass 2:    xoshiro256** K=21 seeds 0, 1, and 2 -- the same K=21
           ...
           K=21 pick for draw #2133 is computed with the same verified
...
Pass 6:    "three consecutive pairs" filter. Removes any Pass-5-
           remaining combo whose sorted main numbers decompose into
           exactly three consecutive pairs (each run of exactly 2, no
           run of 3+, no isolated singles) -- e.g. 1,2,9,10,15,16.
...
Pass 10:   "any of the last 100 draws" broad high-overlap filter
           (final). Removes any Pass-9-remaining combo that shares
           exactly 3, 4, or 5 numbers with ANY of the 100 actual draws
           immediately preceding the target (#2033-2132) -- broader
           than Passes 7-9, which each checked exactly one past draw.
           A combo is removed if it overlaps 3+ with even a single one
           of these 100 draws. (Widened from an initial 50-draw window
           to 100 per explicit request, after confirming via a pooled
           1-to-100-steps-back analysis that the underlying pattern
           looks the same at 100 as at 50 -- see caveat below.)
           ...
           expected 57.9 out of 2,131 pairs, p=0.071). Worse, a pooled
           analysis -- every draw in the last 1,000 checked against
           each of its own 1-to-100-steps-back prior draws, ~100,000
           pooled comparisons -- found the 3+/4+/5+ overlap rate
           matches pure chance almost exactly: 2.685% observed vs.
           2.717% chance for 3+, 0.163% observed vs. 0.168% chance for
           4+, and 0.002% observed vs. ~0.004% chance for 5+. That
           pooled result is exactly what Pass 10 draws its filter
           from, which means Pass 10 is -- more clearly than Passes
           7-9 -- removing combos based on a pattern statistically
           indistinguishable from random chance, not a real signal.
           Widening from 50 to 100 draws does NOT change this
           conclusion (the pooled rate is essentially identical at
           both window sizes) -- it only means MORE combos get removed
           on the same non-signal, cutting the final pool from 105,009
           (50-draw window) down to roughly 24,839 (100-draw window).
           All four passes (7/8/9/10) should be read as weak/
           unconfirmed heuristics, not validated statistical edges.

Self-checks the xoshiro implementation against a known-good value before
trusting Base's xoshiro component.

Outputs:
  xoshiro_elim_2133_meta.json          -- small: pools, method picks, counts
  public/xoshiro_elim_2133_combos.json -- large: remaining combo list
                                          (fetched client-side, not inlined)

Run: python precompute_xoshiro_elim_2133.py
"""
...
META_OUT = BASE + r"\xoshiro_elim_2133_staging_meta.json"
COMBOS_OUT = BASE + r"\public\xoshiro_elim_2133_staging_combos.json"
...
# ── Fetch all real draws through #2132 ───────────────────────────────────────
...
# STAGING: Base = xoshiro K=38 alone, Modular Cycle intersection DROPPED
# (Modular Cycle's own K=33 pick is still computed above -- and Modular
# Cycle is still one of the 16 methods used in Pass 1 below -- it's just
# no longer folded into Base construction).
base_pool = sorted(set(xo_pool))
K_BASE = len(base_pool)
print(f"\nBase (STAGING: xoshiro K={K_XO} seed #{SEED_XO:,} ALONE, no Modular Cycle intersection): {K_BASE} numbers: {base_pool}")
...
# Pass-6-remaining combo that shares exactly 3, 4, or 5 numbers with the
# immediately previous actual draw (#2132: 9,15,16,19,31,32) -- based on
# the site's own consecutive-draw-overlap analysis across all 2,131 real
# draw pairs, where 3+ overlaps landed somewhat rarer than the
# independent-draw chance expectation. ──────────────────────────────────
print(f"\n=== Pass 7 ===")
PREV_DRAW_SERIAL = TARGET_SERIAL - 1  # 2132
PREV_DRAW_NUMS = sorted([9, 15, 16, 19, 31, 32])
...
print(f"  Removed (overlap 3, 4, or 5 with draw #{PREV_DRAW_SERIAL}): {len(removed_by_pass7):,}")
...
# ── Pass 10: "any of the last 100 draws" broad high-overlap filter (final).
# Removes any Pass-9-remaining combo that shares exactly 3, 4, or 5 numbers
# with ANY of the 100 actual draws immediately preceding the target
# (#2033-2132) -- broader than Passes 7-9, which each checked exactly one
# past draw. Widened from 50 to 100 per explicit request. See docstring for
# the pooled-analysis caveat. ────────────────────────────────────────────
print(f"\n=== Pass 10 ===")
PASS10_WINDOW_LO = TARGET_SERIAL - 100  # 2033
...
print(f"Last 100 draws before #{TARGET_SERIAL}: #{PASS10_WINDOW_LO}-{PASS10_WINDOW_HI} ({len(pass10_draws)} draws)")
...
print(f"  Max-overlap-vs-any-of-100 distribution (of Pass-9-remaining combos): " +
      ", ".join(f"{k}:{v:,}" for k, v in sorted(pass10_max_overlap_dist.items())))
print(f"  Removed (overlap 3, 4, or 5 with at least one of the last 100 draws): {len(removed_by_pass10):,}")
...
meta = {
    ...
    'trainedThroughSerial': train_serials[-1],  # 2132 -- equals TARGET_SERIAL-1 for this standard walk-forward build
    ...
    # STAGING: the K=38 universe makes removed_by_pass10 ~2.1M combos --
    # storing the full list blew meta.json past GitHub's 100MB limit.
    # Store just the count + a small preview (gen script only ever uses
    # len() and [:5] on this list, never the full thing).
    'removedByPass10Count': len(removed_by_pass10),
    'removedByPass10Preview': [list(c) for c in removed_by_pass10[:20]],
