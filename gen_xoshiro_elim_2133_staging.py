"""
gen_xoshiro_elim_2133_staging.py
-------------------------------------
STAGING VARIANT of gen_xoshiro_elim_2133.py -- for side-by-side
comparison only. Does NOT touch the live public/xoshiro_elim_2133.html
at all; writes public/xoshiro_elim_2133_staging.html instead. Not
linked from site-nav.js -- reachable only via direct URL, for manual
review before a decision on whether to promote it to replace the live
page (re-run precompute_xoshiro_elim_2133.py + gen_xoshiro_elim_2133.py
with the same Base change, or just keep both indefinitely) or discard
it (delete the four _staging-suffixed files and this script).

The ONLY methodological difference from the live page: Base is
xoshiro256** K=38 seed #692,809's pick ALONE (38 numbers) instead of
that pick intersected with Modular Cycle's K=33 pick (28 numbers on
the live page). See precompute_xoshiro_elim_2133_staging.py's
docstring for the historical-containment rationale behind testing
this. Reads xoshiro_elim_2133_staging_meta.json (small: pool picks,
method picks, counts) produced by precompute_xoshiro_elim_2133_staging.py.
The large remaining-combo list lives separately at
public/xoshiro_elim_2133_staging_combos.json (already written by the
precompute script) and is fetched client-side, not inlined.

Since Base = xoshiro K=38 alone here, Base IS the xoshiro pick --
there is no separate intersection step to display or verify. The
xoshiro pick is recomputed live in the browser (bit-exact BigInt port,
verified against the server-embedded reference) and used directly as
Base. Modular Cycle's K=33 pick is still shown for reference (it's
still one of Pass 1's 16 methods, unrelated to Base here), but is
explicitly labeled as NOT contributing to Base in this build.
...
removedByPass6/finalRemainingPass6 (formerly pass7...) keys.

Extended (2026-08-31) with a new Pass 7 (final) -- consecutive-draw
high-overlap filter vs. the immediately previous actual draw (#2132),
added after the site's own consecutive-draw-overlap analysis found
3+ overlaps between adjacent Loto6 draws to be somewhat rarer than
independent-draw chance. Reads pass7PrevDrawSerial/pass7PrevDrawNums/
removedByPass7/pass7OverlapDistribution/finalRemaining from meta.json.

Output: public/xoshiro_elim_2133_staging.html
Run: python gen_xoshiro_elim_2133_staging.py
"""
import json

BASE = r"C:\Users\Zaw Min Htoon\source\repos\theonelotto"
META_PATH = BASE + r"\xoshiro_elim_2133_staging_meta.json"
HTML_OUT = BASE + r"\public\xoshiro_elim_2133_staging.html"
...
final_remaining_pass9 = meta['finalRemainingPass9']
pass9_pct = final_remaining_pass9 / universe_count * 100
pass9_pct_of_pass8 = final_remaining_pass9 / final_remaining_pass8 * 100

pass10_window_lo = meta['pass10WindowLo']
pass10_window_hi = meta['pass10WindowHi']
pass10_draws = meta['pass10Draws']
# STAGING: full removedByPass10 list isn't stored (too large for the
# K=38 universe -- ~2.1M combos) -- meta only has the count + a preview.
removed_by_pass10_count = meta['removedByPass10Count']
removed_by_pass10_preview = meta['removedByPass10Preview']
pass10_max_overlap_distribution = meta['pass10MaxOverlapDistribution']
final_remaining = meta['finalRemaining']
final_pct = final_remaining / universe_count * 100
pass10_pct_of_pass9 = final_remaining / final_remaining_pass9 * 100
...
<title>🧪 STAGING — Xoshiro K=38 (standalone) + 16-Method Elimination — Draw #{TARGET_SERIAL}</title>
<style>
...
<div class="wrap">
  <div style="background:#450a0a;border:1px solid #7f1d1d;border-radius:10px;padding:14px 18px;margin-bottom:20px;font-size:.85rem;color:#fca5a5;line-height:1.6">
    🧪 <strong>STAGING BUILD — not the live page.</strong> This is an experimental variant of
    <a href="/xoshiro_elim_2133.html" style="color:#fca5a5;text-decoration:underline">xoshiro_elim_2133.html</a> for side-by-side review only —
    it is not linked from site navigation and the live page is completely unaffected by it. The only change: <strong>Base</strong> here is
    xoshiro256** K=38 seed #692,809's pick <strong>alone</strong> (38 numbers), with the Modular Cycle K=33 intersection step dropped
    (live page's Base is the 28-number intersection of the two). Every other pass (1&ndash;10) uses the identical methodology as the live
    page. Compare the two, then either promote this build (re-run the precompute/gen scripts with the live page's filenames) or discard it
    (delete the four <code>_staging</code>-suffixed files).
  </div>
  <h1>✂️ Xoshiro K=38 (standalone) + 16-Method Elimination — Draw #{TARGET_SERIAL}</h1>
  <p class="subtitle">Combinatorial set-difference: xoshiro K=38 pick alone (no Modular Cycle intersection), minus combos covered by any of the 16 prediction methods' K={method_k} picks</p>

  <div class="note">
    <p><strong style="color:#e2e8f0">Base</strong> (below) is <strong>xoshiro256** K={xo['k']} seed #{xo['seed']:,}'s pick alone</strong>
    &mdash; for draw #{TARGET_SERIAL}, current overall best K={xo['k']} seed (0&ndash;1,000,000 scan). <strong style="color:#fca5a5">This
    staging build drops the Modular Cycle intersection the live page uses</strong> &mdash; Modular Cycle's K={mc['k']} pick is shown below
    for reference (it's still one of Pass 1's 16 methods) but does not contribute to Base here. It defines the working universe: all
    C({base['k']},6) = {universe_count:,} six-number combinations drawable from this pool &mdash; about {universe_count/376740:.1f}x
    the live page's C(28,6) = 376,740.</p>
    ...
    history" pattern used in the earlier #2124 elimination flow. Any Pass-2-remaining combo that exactly matches an actual 6-number
    winning combo from any of the {historical_draw_count:,} real draws (#1&ndash;{TRAINED_THROUGH}) gets removed, leaving {final_remaining_pass3:,}.
    ...
    rare pattern. Any Pass-4-remaining combo with max consecutive run &ge;3 gets removed, leaving {final_remaining_pass5:,}.</p>
    ...
    <p><strong style="color:#e2e8f0">Pass 9</strong> is the same heuristic again, "3 steps back" &mdash; against draw
  ...
  <div class="section">
    <h2>Base — Xoshiro K={xo['k']} seed #{xo['seed']:,} <strong style="color:#fca5a5;font-size:.7rem">(STANDALONE, no Modular Cycle intersection)</strong> <span id="badgeBase" class="verify-badge pending">verifying…</span></h2>
    <p class="desc">Xoshiro: current overall best K=38 seed (0–1,000,000 scan), computed live below. In this staging build, <strong>Base = the
    xoshiro pick directly</strong>, {base['k']} numbers &mdash; no intersection step. Modular Cycle's K={mc['k']} pick is shown below for
    reference only (it is still one of Pass 1's 16 methods, just not part of Base here).</p>
    ...
    <h3>Base <span class="order-hint">(= the xoshiro pick above, no intersection applied in this build)</span></h3>
    <div class="balls" id="baseBalls"></div>
    <h3 style="margin-top:20px;color:#64748b">Modular Cycle K={mc['k']} pick <span class="verify-badge na">reference only — NOT part of Base here</span></h3>
    <div class="order-label">Ascending order</div>
  ...
  <div class="section">
    <h2>Pass 9 — "3 steps back" high-overlap filter <span id="badgePass9" class="verify-badge pending">verifying…</span>
    <span class="verify-badge" style="background:#450a0a;color:#fca5a5">not independently tested</span></h2>
    ...
    {', '.join(str(tuple(c)) for c in removed_by_pass9[:5])}{', ...' if len(removed_by_pass9) > 5 else ''}.</p>
  </div>
  ...
  <div class="section">
    ...
    Removed {removed_by_pass10_count:,} combos (overlap 3, 4, or 5 with at least one of the {len(pass10_draws)} draws) &mdash; e.g.
    {', '.join(str(tuple(c)) for c in removed_by_pass10_preview[:5])}, .... Final remaining:
  ...
  <div class="section">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        <div class="lbl">After Pass 9</div>
        <div class="val">{final_remaining_pass9:,}</div>
      ...
      <div class="stat-card">
        ...
        <div class="val">{removed_by_pass10_count:,}</div>
      ...
      <div class="stat-card final">
        ...
        <div class="sub">{final_pct:.1f}% of universe · {pass10_pct_of_pass9:.1f}% of Pass-9 output</div>
      </div>
    </div>
    <div class="elim-flow">
      ...
      <span class="arrow">&rarr;</span>
      <span class="n">{final_remaining_pass9:,}</span> <span style="color:#64748b;font-size:.7rem">(Pass 9)</span>
      <span class="arrow">&rarr;</span>
      <span class="n final">{final_remaining:,}</span> <span style="color:#64748b;font-size:.7rem">(Pass 10)</span>
    </div>
  </div>
...
// Reference only in this staging build -- NOT used to construct Base. ──────
...
// ── Base (STAGING): the live xoshiro pick directly, no Modular Cycle
// intersection -- Base and the xoshiro pick are the same set here. ─────────
const liveBase = liveXo.slice().sort((a, b) => a - b);
...
function pass10MaxOverlap(combo) {{
  return Math.max(...PASS10_DRAW_SETS.map(s => combo.filter(n => s.has(n)).length));
}}
function pass10HighOverlapAny(combo) {{
  return PASS10_DRAW_SETS.some(s => {{
    const ov = combo.filter(n => s.has(n)).length;
    return ov === 3 || ov === 4 || ov === 5;
  }});
}}
...
fetch('/xoshiro_elim_{TARGET_SERIAL}_staging_combos.json')
  .then(r => r.json())
  .then(data => {{
    REMAINING = data;
    filtered = REMAINING;
...
function downloadCSV() {{
  ...
  a.href = url;
  a.download = 'draw_{TARGET_SERIAL}_staging_remaining_combos.csv';
  a.click();
