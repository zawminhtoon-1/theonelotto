"""
gen_custom_avg.py
Strategy: Two-Draw Average
Sort the 7 numbers (6 main + bonus) from each of the last two draws.
Pair them by position, average each pair, round to nearest integer.
Clamp to 1-43 and deduplicate. These are the 7 predicted numbers.
...
N = len(draws)
test_start = max(2, N - BT_DRAWS)  # need 2 prior draws
...
<title>Two-Draw Average Predict — Loto 6</title>
<style>
...
.method-card .mc-title{{font-size:.78rem;font-weight:700;color:#34d399;
...
.an-pred{{background:#34d399;color:#0f172a}}
...
.ball{{width:52px;height:52px;border-radius:50%;display:flex;align-items:center;
  justify-content:center;font-size:1.1rem;font-weight:800;color:#0f172a;
  background:#34d399;border:3px solid #10b981;cursor:default;transition:.12s}}
  ...
  <div class="nav-groups">
    ...
    <div class="nav-group">
      ...
      <div class="nav-dropdown"><div class="nav-dropdown-inner">
        ...
        <a href="/custom_avg.html" class="active">&#10133; Two-Draw Avg</a>
      </div></div>
    </div>
    <div class="nav-group">
...
<main>
  ...
  <div class="method-card">
    ...
    <div class="mc-body">
      Sort all 7 numbers (6 main + bonus) from each of the two most recent draws.
      Average each pair by position. Round to the nearest integer, clamp to 1&ndash;43,
  ...
  tbody.innerHTML += `
    ...
    <tr class="pred-row">
      <td colspan="5" style="padding:4px 10px 10px">
        <div style="font-size:.65rem;color:#475569;margin-bottom:3px">Predicted 7:</div>
...
out = os.path.join(os.path.dirname(__file__), "public", "custom_avg.html")
