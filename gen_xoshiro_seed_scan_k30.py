"""
gen_xoshiro_seed_scan_k30.py
--------------------------------
Static report page for the K=30 xoshiro256** backtest scan -- the K=30
companion to xoshiro_seed_scan_k20.html: same seed range -3,000,000 to
3,000,000 (6,000,001 seeds), same draws #1-2050 (2050 draws, matched
exactly to the K=20 scan's window for direct comparability), ranked by
...
coverage is currently in loto6_local.db's seed_hit_xoshiro_k30 table
(populated by load_xoshiro_seed_scan_k30_to_db.py) and renders a
...
Output: public/xoshiro_seed_scan_k30.html
Run: python gen_xoshiro_seed_scan_k30.py
"""
...
HTML_OUT = BASE + r"\public\xoshiro_seed_scan_k30.html"
...
if num_seeds == 0:
    raise SystemExit(f"{TABLE} is empty -- run load_xoshiro_seed_scan_k30_to_db.py first.")
...
<title>Xoshiro Seed Scan K=30 (±3,000,000){title_suffix} — Loto 6</title>
...
<div class="wrap">
  <h1>🎯 Xoshiro Seed Scan — K=30 (seeds -3,000,000 to 3,000,000)</h1>
  ...
  <div class="note">
    <p>K=30 companion to <a href="/xoshiro_seed_scan_k20.html" style="color:#a78bfa">the K=20 seed scan</a> &mdash; same
    xoshiro256**/SplitMix64 algorithm and combined-seed formula (<code>seed×10,000,000 + draw_serial</code>), same seed
    range {FULL_SEED_LO:,} to {FULL_SEED_HI:,} ({FULL_EXPECTED:,} values), same draw window #{DRAW_START}–{DRAW_END}
    ({N_DRAWS} draws, matched exactly to K=20's window for direct comparability between the two scans), split into the
    same 6 stages of 1,000,001 seeds each. Only difference: {K_PICKS} picks instead of 20. Self-checked against
    independently-computed known-good reference vectors (including negative seeds) before scaling, per stage.</p>
    ...
    chance rates run much higher than K=20's tight pool &mdash; hypergeometric expectation is hit6b&asymp;{exp_hit6b:.0f},
    hit6&asymp;{exp_hit6:.0f}, hit5&asymp;{exp_hit5:.0f}, hit4&asymp;{exp_hit4:.0f} per seed (of {N_DRAWS} draws) &mdash;
    but all four tiers are still tracked for the same ranking convention: highest hit6b, tiebreak hit6, tiebreak hit5,
    tiebreak hit4.</p>
  </div>
  ...
  <p class="footer">
    ...
    Algorithm verified against independent reference sources before running, plus a
