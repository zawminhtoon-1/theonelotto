"""
gen_pcg64_seed_scan_k38.py
--------------------------------
Static report page for the PCG64 K=38 backtest scan -- the PCG64
...
Seed range -5,000,000 to 5,000,000 (10,000,001 seeds, wider than the
xoshiro scans' -3M to 3M per explicit request), same draws #1-2050
(2050 draws, matched exactly to the xoshiro scans' window for direct
comparability), ranked by hit6b (6-hit + bonus) desc, tiebreak hit6
desc, hit5 desc, hit4 desc.

Designed to be regenerated INCREMENTALLY after every stage of the
10-stage scan completes (not just once at the end) -- reads whatever
coverage is currently in loto6_local.db's seed_hit_pcg64_k38 table
(populated by load_pcg64_seed_scan_k38_to_db.py) and renders a "scan
in progress" banner with the current stage count / seed coverage /
...
per-draw breakdowns work for ANY seed typed in, computed live via a
bit-exact BigInt PCG64 JS port (verified against the Python core,
which was itself verified bit-exact against numpy.random.Generator
(PCG64()) via direct low-level state injection before this scan ever
ran).

Output: public/pcg64_seed_scan_k38.html
Run: python gen_pcg64_seed_scan_k38.py
"""
...
HTML_OUT = BASE + r"\public\pcg64_seed_scan_k38.html"
...
if num_seeds == 0:
    raise SystemExit(f"{TABLE} is empty -- run load_pcg64_seed_scan_k38_to_db.py first.")
...
<title>PCG64 Seed Scan K=38 (±5,000,000){title_suffix} — Loto 6</title>
...
<div class="wrap">
  <h1>🎯 PCG64 Seed Scan — K=38 (seeds -5,000,000 to 5,000,000)</h1>
  <p class="subtitle">{num_seeds:,} seeds scanned so far · K={K_PICKS} picks · {N_DRAWS} draws (#{DRAW_START}–{DRAW_END}, nearly full Loto6 history) · PCG64 (O'Neill XSL-RR 128/64, SplitMix64-expanded)</p>
  ...
  <div class="note">
    <p>PCG64 companion to <a href="/xoshiro_seed_scan_k30.html" style="color:#a78bfa">the xoshiro256** K=20/K=30 scans</a>
    &mdash; testing whether swapping the underlying <strong>PRNG family</strong> (not just K) changes anything descriptively.
    Same draw window #{DRAW_START}&ndash;{DRAW_END} ({N_DRAWS} draws) and the same combined-seed formula
    (<code>seed&times;10,000,000 + draw_serial</code>) as every xoshiro scan, but a wider seed range &mdash;
    {FULL_SEED_LO:,} to {FULL_SEED_HI:,} ({FULL_EXPECTED:,} values, split into {N_STAGES} stages of ~1,000,000 each).</p>
    ...
    for their 256-bit state, just producing 128+128 bits instead of 4&times;64 &mdash; this is this site's own seeding
    convention, chosen to stay JS-portable rather than porting NumPy's heavier SeedSequence hash-mixing). Each next-value
    call then runs O'Neill's standard PCG64 XSL-RR construction: a 128-bit LCG advance
    (<code>state = state&times;0x2360ed051fc65da44385df649fccf645 + inc (mod 2&sup1;&sup2;&sup8;)</code>) followed by a
    64-bit xor-shift + random-rotate output permutation. K={K_PICKS} picks come from the same partial Fisher-Yates over
    range(1,44) as every other scan on this site.</p>
    <p><strong style="color:#e2e8f0">Verification:</strong> the pure-Python core used for this scan was checked bit-exact
    against <code>numpy.random.Generator(PCG64())</code> itself &mdash; by injecting our own <code>{{state, inc}}</code>
    directly into NumPy's bit generator (bypassing SeedSequence) and comparing 10 consecutive raw outputs, which matched
    exactly &mdash; before this scan was ever run. Self-checked per stage against independently-computed known-good
    reference vectors (including negative seeds) before scaling, same discipline as every other scan.</p>
    ...
    <b>hit4</b> = exactly 4 of 6. At K={K_PICKS} (a {K_PICKS}-of-{LOTO6_MAX} pool, ~{K_PICKS/LOTO6_MAX*100:.0f}% coverage)
    chance rates run very high &mdash; hypergeometric expectation is hit6b&asymp;{exp_hit6b:.0f}, hit6&asymp;{exp_hit6:.0f},
    hit5&asymp;{exp_hit5:.0f}, hit4&asymp;{exp_hit4:.0f} per seed (of {N_DRAWS} draws) &mdash; but all four tiers are still
    tracked for the same ranking convention: highest hit6b, tiebreak hit6, tiebreak hit5, tiebreak hit4.</p>
  </div>
  ...
  <p class="footer">
    PCG64 (O'Neill XSL-RR 128/64, seeded via SplitMix64-expanded {{state,inc}}): picks = partial Fisher-Yates(range(1,44), {K_PICKS})
    with combined seed = seed×10⁷ + draw_serial. Core algorithm verified bit-exact against <code>numpy.random.Generator(PCG64())</code>
    via direct low-level state injection (bypassing SeedSequence) before this scan ran.<br>
