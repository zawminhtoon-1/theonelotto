"""
load_xoshiro_seed_scan_k20_to_db.py
--------------------------------------
Combines whichever stages of the K=20 scan currently exist on disk
(xoshiro_seed_scan_k20_stage1.json through stage6.json -- run
INCREMENTALLY as each stage completes, not necessarily all 6 present
yet) into loto6_local.db as seed_hit_xoshiro_k20, mirroring the other
seed_hit_xoshiro_k* tables' schema pattern. Unlike the K=33/K=35/K=38
loaders (which assume the full final range is already scanned),
this one is designed to be re-run after EVERY stage completes, so the
...
Run: python load_xoshiro_seed_scan_k20_to_db.py
"""
...
stage_files = sorted(
    glob.glob(BASE + r"\xoshiro_seed_scan_k20_stage*.json"),
    key=lambda p: int(re.search(r"stage(\d+)\.json$", p).group(1))
)
if not stage_files:
    raise SystemExit("No xoshiro_seed_scan_k20_stage*.json files found -- run a stage first.")
