"""
gen_xoshiro_seed_scan_loto7_k30.py
--------------------------------------
Static report page for Loto7's xoshiro256** K=30 seed scan: seeds
-1,000,000 to 1,000,000 (2,000,001 seeds), K=30 picks, backtested
against the FIRST 650 Loto7 draws (#1-650) -- direct reuse of the
K=28 scan's infrastructure and page pattern
(gen_xoshiro_seed_scan_loto7_k28.py), K_PICKS changed from 28 to 30,
draw window widened from #1-500 to #1-650 (per explicit user request),
and top-3 best-seed summary (reverting to K=25's original top-3
convention, per explicit user request for "best 3 seeds").

Run directly at the full +/-1,000,000 range via
xoshiro_seed_scan_loto7_k30_stage.py, staged in 4 ~500,000-seed
batches (same staged negative-seed pattern as the Loto6 K=35 scan and
the Loto7 K=25/K=28 scans), each stage self-checked (including
negative seeds) before running.

NEW for this page: an in-sample vs out-of-sample breakdown for the
top 3 seeds. The scan itself only trains/ranks on #1-650 (in-sample);
draws #651-691 (the real draws that have landed since, 41 draws) were
deliberately held out and are checked here separately, to test whether
the "best" seeds found by brute-force search over 2,000,001 candidates
actually generalize -- or whether their in-sample edge is just a
selection-bias artifact (picking the best of millions will always beat
chance in-sample by construction, whether or not there's a real signal).

Reads live from loto7_local.db's seed_hit_xoshiro_k30 table for
aggregates and the top-N table. Draw records for the client-side
seed-detail modal (and the out-of-sample check) are pulled directly
from loto7_results (Neon Postgres), same live-lookup pattern as the
Loto6 page.
...
Output: public/xoshiro_seed_scan_loto7_k30.html
Run: python gen_xoshiro_seed_scan_loto7_k30.py
"""
...
HTML_OUT = BASE + r"\public\xoshiro_seed_scan_loto7_k30.html"
...
OOS_START, OOS_END = 651, 691
N_OOS_DRAWS = OOS_END - OOS_START + 1  # 41
...
exp_hit7b_oos = p_hit7b * N_OOS_DRAWS
exp_hit7_oos = p_hit7_only * N_OOS_DRAWS
...
print(f"Analytical expectation (chance): hit7b~={exp_hit7b:.3f} ({p_hit7b*100:.2f}%/draw) hit7~={exp_hit7:.2f} "
      f"hit6~={exp_hit6:.2f} hit5~={exp_hit5:.2f} hit4~={exp_hit4:.2f} (of {N_DRAWS} draws)")
print(f"Top 3: {[(r[0], r[1], r[2], r[3], r[4], r[5]) for r in top3]}")

# ── Load draw window #1-650 (in-sample) AND #651-691 (out-of-sample, held
# out of the scan entirely) from the production DB. ──────────────────────────
...
oos_pg_rows = pgcur.fetchall()
pg.close()
if len(pg_rows) != N_DRAWS:
    raise SystemExit(f"Draw window mismatch: got {len(pg_rows)} rows, expected {N_DRAWS}")
if len(oos_pg_rows) != N_OOS_DRAWS:
    raise SystemExit(f"Out-of-sample draw window mismatch: got {len(oos_pg_rows)} rows, expected {N_OOS_DRAWS}")
DRAWS = [{'s': r[0], 'd': r[1].isoformat() if r[1] else None, 'a': list(r[2:9]), 'b1': r[9], 'b2': r[10]} for r in pg_rows]
OOS_DRAWS = [{'s': r[0], 'd': r[1].isoformat() if r[1] else None, 'a': list(r[2:9]), 'b1': r[9], 'b2': r[10]} for r in oos_pg_rows]
js_draws = json.dumps(DRAWS, separators=(',', ':'))
js_oos_draws = json.dumps(OOS_DRAWS, separators=(',', ':'))
print(f"Loaded {len(DRAWS)} in-sample draw records (#{DRAWS[0]['s']}-{DRAWS[-1]['s']}) "
      f"and {len(OOS_DRAWS)} out-of-sample draw records (#{OOS_DRAWS[0]['s']}-{OOS_DRAWS[-1]['s']}).")

# ── In-sample vs out-of-sample check for the top 3 seeds (server-side,
# Python reference -- the page's JS recomputes the same thing live). ─────────
MASK64 = 0xFFFFFFFFFFFFFFFF
def xoshiro_predict(seed, draw_serial, k, pool_max=LOTO7_MAX):
...
for row in top3:
    seed = row[0]
    hit7b = hit7 = hit6 = hit5 = hit4 = 0
    for d in OOS_DRAWS:
        picks = set(xoshiro_predict(seed, d['s'], K_PICKS))
        actual = set(d['a'])
        h = len(actual & picks)
        if h == 7:
            hit7 += 1
            if d['b1'] in picks or d['b2'] in picks:
                hit7b += 1
        elif h == 6: hit6 += 1
        elif h == 5: hit5 += 1
        elif h == 4: hit4 += 1
    in_pct = row[1] / N_DRAWS * 100
    oos_pct = hit7b / N_OOS_DRAWS * 100
    oos_results.append({
        'seed': seed, 'inHit7b': row[1], 'inHit7': row[2], 'inHit6': row[3], 'inHit5': row[4], 'inHit4': row[5],
        'inPct': in_pct, 'oosHit7b': hit7b, 'oosHit7': hit7, 'oosHit6': hit6, 'oosHit5': hit5, 'oosHit4': hit4,
        'oosPct': oos_pct,
    })
    print(f"  seed={seed:>9,}  in-sample hit7b={row[1]:3d} ({in_pct:.1f}%/draw)  "
          f"out-of-sample hit7b={hit7b:2d}/{N_OOS_DRAWS} ({oos_pct:.1f}%/draw)  chance={p_hit7b*100:.1f}%/draw")
...
for i, r in enumerate(oos_results, 1):
    below_chance = r['oosPct'] < p_hit7b * 100
    oos_cards_html += f"""<div class="stat-card{' warn' if below_chance else ''}">
      <div class="lbl">#{i} seed &#183; {r['seed']:,}</div>
      <div class="val">{r['inPct']:.1f}% <span style="font-size:.6em;color:#64748b">in-sample</span> &rarr; {r['oosPct']:.1f}% <span style="font-size:.6em;color:#64748b">out-of-sample</span></div>
      <div class="sub">in: hit7b {r['inHit7b']}/{N_DRAWS} &middot; out: hit7b {r['oosHit7b']}/{N_OOS_DRAWS} hit7 {r['oosHit7']} hit6 {r['oosHit6']} hit5 {r['oosHit5']} hit4 {r['oosHit4']}
      {"&mdash; <strong style='color:#f87171'>below chance (" + f"{p_hit7b*100:.1f}%" + ")</strong>" if below_chance else "&mdash; <strong style='color:#4ade80'>above chance</strong>"}</div>
    </div>"""
...
<title>Xoshiro Seed Scan K=30 (-1,000,000 to 1,000,000) — Loto 7</title>
...
.stat-card.warn{{border-color:#f8717188;background:#1c0a0a}}
...
<div class="wrap">
  <h1>🎯 Xoshiro Seed Scan — Loto 7 K=30 (&minus;1,000,000 to 1,000,000)</h1>
  ...
  <div class="note">
    Reuses the <a href="/xoshiro_seed_scan_loto7_k28.html" style="color:#a78bfa">Loto7 K=28 scan</a>'s infrastructure
    directly at the full {SEED_LO:,} to {SEED_HI:,} range ({num_seeds:,} seeds, K={K_PICKS} instead of K=28) &mdash;
    same staged negative-seed pattern already verified on <a href="/xoshiro_seed_scan_k35.html" style="color:#a78bfa">the Loto6 K=35 scan</a>:
    run in 4 batches of ~500,000 seeds each (Python's bitwise AND on negative integers correctly produces the same
    64-bit two's-complement result as unsigned arithmetic, self-checked against a from-scratch modular reference
    &mdash; including negative-seed cases &mdash; before each batch). Same xoshiro256**/SplitMix64 algorithm and
    combined-seed formula (<code>seed&times;10,000,000 + draw_serial</code>) reused exactly from the Loto6 scans,
    reparameterized for Loto7's 37-number pool (not 43) and K={K_PICKS} picks. Backtested against the <b>FIRST</b>
    {N_DRAWS} Loto7 draws (#{DRAW_START}&ndash;{DRAW_END}) &mdash; not the most recent (widened from the K=25/K=28
    scans' #1&ndash;500 window per explicit request).
    <br><br>
  ...
  <div class="section">
    <h2>Best 3 seeds</h2>
    <p class="desc">Ranked hit7b &rarr; hit7 &rarr; hit6 &rarr; hit5 &rarr; hit4, in-sample (#{DRAW_START}&ndash;{DRAW_END}).</p>
    <div class="stats-row">{top3_summary_html}</div>
  </div>

  <div class="section">
    <h2>In-sample vs out-of-sample: does the "edge" survive?</h2>
    <p class="desc">The scan above only ever sees draws #{DRAW_START}&ndash;{DRAW_END} (in-sample) when ranking seeds. Draws
    #{OOS_START}&ndash;{OOS_END} ({N_OOS_DRAWS} real draws that have since landed) were held out entirely and are checked
    here for the first time, against these same top 3 seeds &mdash; the standard test for whether a "best of millions"
    result is a genuine edge or just <a href="/xoshiro_seed_scan_loto7_k28.html" style="color:#a78bfa">the same
    overfitting pattern</a> already seen on the K=25/K=28 scans (picking the best of 2,000,001 candidates will always
    beat chance in-sample by construction &mdash; that alone proves nothing about #{OOS_START}+).</p>
    <div class="stats-row">{oos_cards_html}</div>
    <p class="desc" style="margin-top:8px;margin-bottom:0">Chance baseline: hit7b &asymp; {p_hit7b*100:.2f}%/draw
    (hypergeometric, K={K_PICKS} of {LOTO7_MAX}) &mdash; expected {exp_hit7b_oos:.2f} hit7b out of {N_OOS_DRAWS}
    out-of-sample draws by pure chance alone. A seed whose out-of-sample rate lands at or below this line has no
    demonstrated edge, regardless of how far above chance it scored in-sample.</p>
  </div>
  ...
  <p class="footer">
    ...
    (in-sample) and #{OOS_START}–{OOS_END} (out-of-sample) sourced directly from the production database
    (<code>loto7_results</code>).<br>
...
// Combines in-sample + out-of-sample draws so the modal shows the full
// #{DRAW_START}-{OOS_END} history for any seed looked up.
const DRAWS = {js_draws}.concat({js_oos_draws});
...
function openSeedDetail(seed) {{
  ...
  [...DRAWS].reverse().forEach(row => {{
    ...
    const oosTag = row.s >= {OOS_START} ? ' <span style="color:#64748b;font-size:.65rem">OOS</span>' : '';
    ...
    htmlParts.push(
      '<tr><td style="color:#64748b;white-space:nowrap">' + row.s + oosTag + '</td>' +
