"""
xoshiro_seed_scan_k33_100k_to_1m.py
---------------------------------------
Second extension of the K=33 scan: seeds 100,001-1,000,000 (900,000
seeds), same K=33, same xoshiro256** algorithm/formula, same draw window
#1001-2127 (1127 draws), same three metrics (hit6b, hit6, hit5) and
ranking. Adds to (not replaces) the existing seeds 0-100,000 already in
seed_hit_xoshiro_k33 -- combined, this covers the full 0-1,000,000 range.

10x the previous batch (90,000 seeds -> 900,000 seeds); at the prior
run's ~114 seeds/s (7 workers) this is expected to take ~2.2 hours.
Kept at 7 workers (not bumped to the machine's 8 logical cores) since
other processes (Docker, a Temporal worker, dev tools) are running
concurrently on this machine -- not worth the contention risk over a
run this long for a ~14% theoretical gain.

Draw records pulled from the production Neon Postgres DB (same as the
prior two runs), verified for exactly 1127 consecutive rows before
scanning.

Run: python xoshiro_seed_scan_k33_100k_to_1m.py
"""
...
OUT_JSON = BASE + r"\xoshiro_seed_scan_k33_100k_to_1m.json"

K_PICKS = 33
DRAW_START, DRAW_END = 1001, 2127
N_DRAWS = DRAW_END - DRAW_START + 1  # 1127
LOTO6_MAX = 43
SEED_LO, SEED_HI = 100_001, 1_000_000   # inclusive -- extends the existing 0-100,000 range
N_WORKERS = 7
CHUNK_SIZE = 200
...
def main():
    ...
    for test_seed, test_draw in [(100001, 1001), (71812, 2127), (999999, 1500), (555555, 2000), (1000000, 2127)]:
        ...
        for i, chunk_result in enumerate(pool.imap_unordered(process_chunk, chunks), 1):
            ...
            if i % 25 == 0 or i == total_chunks:
                ...
                print(f"[{i:,}/{total_chunks:,} chunks, {done_seeds:,}/{num_seeds:,} seeds] "
                      f"elapsed={elapsed:.0f}s ({elapsed/60:.1f}min) rate={rate:.0f} seeds/s eta={eta:.0f}s ({eta/60:.1f}min)", flush=True)
    ...
    for r in ranked[:10]:
        print(f"  seed={r[0]:7d}  hit6b={r[1]:3d}  hit6={r[2]:3d}  hit5={r[3]:3d}")
