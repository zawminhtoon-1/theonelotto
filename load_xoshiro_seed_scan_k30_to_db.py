"""
load_xoshiro_seed_scan_k30_to_db.py
--------------------------------------
Combines whichever stages of the K=30 scan currently exist on disk
(xoshiro_seed_scan_k30_stage1.json through stage6.json -- run
INCREMENTALLY as each stage completes, not necessarily all 6 present
yet) into loto6_local.db as seed_hit_xoshiro_k30, mirroring the K=20
loader's pattern exactly. Designed to be re-run after EVERY stage
completes, so the live page can be rebuilt and pushed incrementally
while later stages are still running in the background. Drops and
recreates the table fresh each run (safe -- always rebuilt from the
source stage JSON files, never accumulated in-place).

Full target range once all 6 stages land: seeds -3,000,000 to
3,000,000 (6,000,001 seeds). Reports current coverage/percentage
...
Run: python load_xoshiro_seed_scan_k30_to_db.py
"""
...
stage_files = sorted(
    glob.glob(BASE + r"\xoshiro_seed_scan_k30_stage*.json"),
    key=lambda p: int(re.search(r"stage(\d+)\.json$", p).group(1))
)
if not stage_files:
    raise SystemExit("No xoshiro_seed_scan_k30_stage*.json files found -- run a stage first.")
...
# Loaded stages should each be a contiguous block; verify no internal gaps within
# what's been combined so far (stage N+1 not yet run just means the range stops early).
...
if count == FULL_EXPECTED and lo == FULL_SEED_LO and hi == FULL_SEED_HI:
    ...
    print(f"\nScan IN PROGRESS: {count:,} / {FULL_EXPECTED:,} seeds loaded ({pct:.1f}% of full "
          f"{FULL_SEED_LO:,} to {FULL_SEED_HI:,} range). {len(stage_nums_loaded)}/6 stages done.")
