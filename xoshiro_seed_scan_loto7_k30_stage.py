"""
xoshiro_seed_scan_loto7_k30_stage.py
----------------------------------------
Parameterized stage runner for the Loto7 K=30 xoshiro scan: seeds
-1,000,000 to 1,000,000 (2,000,001 seeds total), split into 4
roughly equal stages of ~500,000 seeds each -- direct reuse of the
K=28 scan's infrastructure (xoshiro_seed_scan_loto7_k28_stage.py),
just K_PICKS changed from 28 to 30 AND the backtest window widened
from #1-500 to #1-650 (650 draws, per explicit user request -- draws
#651-691 held out as an out-of-sample check, done separately after
this scan). Same xoshiro256** algorithm/formula (seed*10,000,000 +
draw_serial), negative seeds handled via Python's arbitrary-precision
ints + & MASK64 (two's-complement wrap, no special-casing needed --
same pattern verified on the Loto6 K=35 scan and the Loto7 K=25/K=28
scans), same hit7b/hit7/hit6/hit5/hit4 metrics. Self-checks (including
negative-seed cases) before scaling, per stage.

Usage: python xoshiro_seed_scan_loto7_k30_stage.py <stage_num> <seed_lo> <seed_hi>
Example: python xoshiro_seed_scan_loto7_k30_stage.py 1 -1000000 -500001

Stage boundaries (same split as K=25/K=28):
  1: -1,000,000 to  -500,001  (500,000 seeds)
  2:   -500,000 to      -1    (500,000 seeds)
  3:          0 to  499,999   (500,000 seeds)
  4:    500,000 to 1,000,000  (500,001 seeds)
"""
...
def main():
    ...
    OUT_JSON = BASE + rf"\xoshiro_seed_scan_loto7_k30_stage{stage_num}.json"
    ...
    test_cases = [(SEED_LO, DRAW_START), (SEED_HI, DRAW_END), (-1000000, 1), (-1, 650), (0, 1), (1000000, 650), (-500000, 325)]
