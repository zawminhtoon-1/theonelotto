"""
gen_miniloto_predictions.py
------------------------------
Computes each of the 16 methods' top-15 candidate pool for the NEXT (not yet
drawn) MiniLoto draw, using ALL historical draws as training data. Separate,
one-shot computation from gen_miniloto_backtest.py's walk-forward evaluation
(fixed at K=5 for strict historical scoring) -- mirrors
gen_loto7_predictions.py's approach.

Output: public/miniloto_predictions_data.json
Run: python gen_miniloto_predictions.py
"""
...
OUT_PATH = BASE + r"\public\miniloto_predictions_data.json"
...
train_main5 = all_main5
target_serial = all_serials[-1] + 1

# ── Helpers ───────────────────────────────────────────────────────────────────
def pad_to_k(base_picks, all_before_main5, k=POOL_K):
...
def make_unique(nums, all_before_main5, k=POOL_K):
    seen = set(); result = []
    for n in nums:
...
def method_ma(window_size=31):
    window = train_main5[-window_size:]
    base = []
    for p in range(5):
...
def method_weighted_ma31(k=POOL_K):
    window = train_main5[-31:]
    n = len(window)
...
def method_lstm():
    vecs = [to_vec31(nums) for nums in train_main5]
    m = LSTM()
...
labeled_fns = [
    ...
    method_naive_bayes, method_weighted_ma31, method_lstm,
]
